...
class Decoder:
    def __init__(
        ...
        self.token_embedding = Embedding(vocab_size, d_model, data_type)
        self.position_embedding = PositionalEncoding(max_length, d_model, dropout, data_type)

        self.layers = [DecoderLayer(d_model, heads_num, d_ff, dropout, data_type) for _ in range(layers_num)]

        self.fc_out = Dense(inputs_num=d_model, units_num=vocab_size, data_type=data_type)
    ...
    def backward(self, error):
        ...
        for layer in reversed(self.layers):
            error, encoder_error = layer.backward(error)
