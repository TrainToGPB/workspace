...
class EncoderLayer:
    ...
    def forward(self, src, src_mask, training):
        _src, _ = self.self_attention.forward(src, src, src, src_mask, training) # Multi-head attention
        src = self.self_attention_norm.forward(src + self.dropout.forward(_src, training)) # Residual + Normalziation

        _src = self.positionwise_feed_forward.forward(src, training) # Feed forward
        src = self.ff_layer_norm.forward(src + self.dropout.forward(_src, training)) # Residual + Normalization
    ...
    def backward(self, error):
        ...
        return _error1 + _error2 + _error3 + error
    ...
    def update_weights(self, layer_num):
        ...
        layer_num = self.positionwise_feed_forward.update_weights(layer_num)
