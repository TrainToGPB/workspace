...
from transformer.layers.base.layer_num import LayerNormalization
...
class DecoderLayer():
    def __init__(self, d_model, heads_num, d_ff, dropout, data_type):
        ...
        self.encoder_attention_norm = LayerNormalization(d_model, epsilon=1e-6, data_type=data_type)
        ...
        self.positionwise_feed_forward = PositionwiseFeedForward(d_model, d_ff, dropout)
    ...
    def forward(self, trg, trg_mask, src, src_mask, training):
        ...
        trg = self.encoder_attention_norm.forward(trg + self.dropout.forward(_trg, training))

        _trg = self.positionwise_feed_forward.forward(trg, training)
    ...
    def backward(self, error):
        ...
        _error = self.positionwise_feed_forward.backward(self.dropout.backward(error))
        error = self.encoder_attention_norm.backward(error + _error)
        ...
        _error1, _error2, _error3 = self.self_attention.backward(self.dropout.backward(error))

        return _error1 + _error2 + _error3 + error, enc_error1 + enc_error2
    

    def set_optimizer(self, optimizer):
        self.self_attention_norm.set_optimizer(optimizer)
        self.encoder_attention_norm.set_optimizer(optimizer)
        ...
        self.positionwise_feed_forward.set_optimizer(optimizer)


    def update_weights(self, layer_num):
        layer_num = self.self_attention_norm.update_weights(layer_num)
        layer_num = self.encoder_attention_norm.update_weights(layer_num)
        ...
        layer_num = self.v.update_weights(layer_num)
