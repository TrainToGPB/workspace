...
class CrossEntropy():
    ...
    def derivative(self, y, t):
        ...
        error = 1 / batch_size
        nll_loss_der = -1 * np.where(np.isin(y, y[np.arange(len(t)), t]), error, 0).astype(y.dtype)
        output_error = self.log_softmax.backward(nll_loss_der)
        
        return np.where(t.reshape(-1, 1) == self.ignore_index, 0, output_error)
